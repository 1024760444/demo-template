...
<body class="gray-bg">
						...
						<div class="row">
							...
							<div class="col-sm-3">
								<div class="input-group">
                                    <input id="search_input" type="text" placeholder="请输入用户名" class="input-sm form-control"> 
                                    <span class="input-group-btn">
                                    	<button type="button" class="btn btn-sm btn-primary glyphicon glyphicon-search"></button>
                                    </span>
                                </div>
							</div>
						</div>
	...
	<script type="text/javascript">
	    $(document).ready(function() {
			...
			$.ajax({
				type : "GET",
				url : "http://localhost:8080/spring-mybatis-hplus/user/scan",
				data : {
					'type' : 1,
				},
				dataType : "json",
			...
			$(".glyphicon-edit").click(function() {
				if($(".checked").length < 1) {
					...
					$("#userPasswd").val("");
					$("#roleId option[value='"+ td4.text() + "'] ").attr("selected",true);
					$("#userEmail").val(td5.text());
					$("#userPhone").val(td6.text());
			...
			$(".delete_post").click(function() {
				$.ajax({
					type : "POST",
					url : "http://localhost:8080/spring-mybatis-hplus/user/delete",
					data : {
			...
			$(".glyphicon-floppy-save").click(function() {
				$.ajax({
					type : "POST",
					url : "http://localhost:8080/spring-mybatis-hplus/user/addOrUpdate",
					data : {
						'id' : $("#id").val(),
